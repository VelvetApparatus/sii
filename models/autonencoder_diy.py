...
from models.linear import LinearDense


class AutoEncoderDIY(nn.Module):
    def __init__(
        self,
        in_channels: int = 1,
        latent_dim: int = 128,
        ...
        for out_dim in hidden_dims:
            encoder_layers.append(LinearDense(in_dim, out_dim))
            encoder_layers.append(nn.ReLU(inplace=True))
            in_dim = out_dim
        encoder_layers.append(LinearDense(in_dim, latent_dim))
        ...
        for out_dim in reversed(hidden_dims):
            decoder_layers.append(LinearDense(in_dim, out_dim))
            decoder_layers.append(nn.ReLU(inplace=True))
            in_dim = out_dim
        decoder_layers.append(LinearDense(in_dim, self.input_dim))
